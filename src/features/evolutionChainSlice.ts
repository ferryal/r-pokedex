...
export const { initialize, error, success, getEvolutionChainReducer } =
...
export const getEvolutionChainByName = wrapReduxAsyncHandler(
  statusHandler,
  async (dispatch, { name }) => {
    const result = await fromApi.getEvolutionChainById(name);
    dispatch(
...
export const getEvolutionChainById = wrapReduxAsyncHandler(
  statusHandler,
  async (dispatch, { id }) => {
    const result = await fromApi.getEvolutionChainById(Number(id));
    dispatch(
      getEvolutionChainReducer({ evolutionChain: camelcaseObject(result) })
    );
  }
